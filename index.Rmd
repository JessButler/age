...
Areas with high density children under 5
=====================================
...
```{r}
#table areas with highest fraction of population under age 5

aberdeen %>%
  select(DZ, IZname, total_population, perc_under_5, SIMD2020v2_Decile) %>%
  filter(perc_under_5 > 7) %>%
  arrange(SIMD2020v2_Decile) %>%
  gt() %>%
  tab_header(
    title = "Most Young Children", 
    subtitle = "Areas with >7% of population under 5, by most deprived") %>%
  cols_label(
    IZname = "Area", 
    total_population = "Population", 
    perc_under_5 = "under 5 (%)",
    SIMD2020v2_Decile = "SIMD")  %>%
  tab_options(container.height = 400)
```
